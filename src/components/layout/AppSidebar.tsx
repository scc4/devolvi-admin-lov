...
import { LayoutDashboard, Users, Truck, LogOut, Building } from "lucide-react";
  ...
  return <Sidebar className={`${isMobile && !open ? "hidden" : ""} border-r`}>
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {menu.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={location.pathname === item.url} className={location.pathname === item.url ? "bg-[#e4e7ff] text-[#6366f1]" : "text-[#2a3547] hover:bg-[#f5f6fa]"} onClick={() => isMobile && useSidebar().setOpen(false)}>
