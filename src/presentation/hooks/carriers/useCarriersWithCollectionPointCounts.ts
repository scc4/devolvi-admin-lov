...
/**
 * Hook to fetch carriers with collection point counts
 */
export function useCarriersWithCollectionPointCounts() {
  ...
  useEffect(() => {
    console.log("useCarriersWithCollectionPointCounts mounted");
    
    return () => {
      console.log("useCarriersWithCollectionPointCounts unmounted");
  ...
  const loadCarriers = useCallback(async (forceRefresh = false) => {
    ...
    console.log("Loading carriers data with collection point counts...");
    ...
    try {
      ...
      console.log(`Retrieved ${carrierDTOs.length} carriers from use case`);
  ...
  const handleCreate = async (carrier: Partial<CarrierDTO>) => {
    ...
    try {
      ...
      if (result.success && result.carrier) {
        await loadCarriers(true); // Force reload carriers to get updated list
